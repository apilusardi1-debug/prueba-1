...
export default function Equipo() {
  ...
  return (
    <div className="p-8">
      ...
      {/* Pestañas */}
      <div className="flex gap-2 mb-6 bg-white rounded-2xl border border-gray-100 shadow-sm p-1.5 w-fit">
        {TABS.map((t) => (
          <button
            key={t.id}
          ...
          >
            {t.icono} {t.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${tabActiva === t.id ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}>
              {datos[t.id]?.length || 0}
            </span>
          </button>
        ))}
      ...
      {cargando ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <button onClick={abrirNuevo} className="mt-4 text-sm text-brand-600 hover:text-brand-800 font-medium underline">
            Agregar el primero
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtrados.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-semibold text-gray-900">{tab.icono} {item.nombre}</p>
                  <a
                    href={`https://wa.me/${item.whatsapp}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-green-600 hover:text-green-700 font-medium mt-0.5 inline-flex items-center gap-1"
                  >
                    💬 {item.whatsapp}
                  </a>
                ...
                <button
                  ...
                  className={`text-xs font-medium px-2.5 py-1 rounded-full shrink-0 ${item.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}
                >

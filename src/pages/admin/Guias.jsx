    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">{guias.length} guías registrados</p>
        </div>
        <button
          onClick={abrirNuevo}
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtrados.map((g) => (
            <div key={g.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 flex items-center justify-between">
              <div>
                <p className="font-semibold text-gray-900">🧭 {g.nombre}</p>
                <p className="text-xs text-gray-400 mt-1">{g.whatsapp}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                <button onClick={() => toggleActivo(g)} className={`text-xs font-medium px-2.5 py-1 rounded-full ${g.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => abrirEditar(g)} className="text-brand-500 hover:text-brand-700 text-xs font-medium">Editar</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setMostrarForm(false)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-lg">{editando ? 'Editar guía' : 'Nuevo guía'}</h2>

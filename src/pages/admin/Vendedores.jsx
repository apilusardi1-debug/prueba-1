  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-100">Vendedores</h1>
          <p className="text-gray-400 dark:text-zinc-500 text-sm">{vendedores.length} vendedores registrados</p>
        </div>
        <button
          onClick={abrirNuevo}
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtrados.map((v) => (
            <div key={v.id} className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm dark:shadow-black/20 px-5 py-4 flex items-center justify-between">
              <div>
                <p className="font-semibold text-gray-900 dark:text-zinc-100">💼 {v.nombre}</p>
                <p className="text-xs text-gray-400 dark:text-zinc-500 mt-1">{v.whatsapp}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                <button onClick={() => toggleActivo(v)} className={`text-xs font-medium px-2.5 py-1 rounded-full ${v.activo ? 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-zinc-800 text-gray-400 dark:text-zinc-500'}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => abrirEditar(v)} className="text-brand-500 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 text-xs font-medium">Editar</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl dark:shadow-black/40 p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-lg text-gray-900 dark:text-zinc-100">{editando ? 'Editar vendedor' : 'Nuevo vendedor'}</h2>
